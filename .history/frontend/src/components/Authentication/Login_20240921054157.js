...
const Login = () => {
  ...
  try {
    ...
    if (response && response.data) {
      ...
      // Clear previous user data and chats
      setLoggedUser(null);  // Clear any existing user
      setChats([]);         // Clear previous chats

      // Save user data to localStorage and update state
      localStorage.setItem("userInfo", JSON.stringify(data));
      setLoggedUser(data);  // Update the logged-in user state

      // Fetch chats for the newly logged-in user (assuming an API endpoint exists)
      const chatsResponse = await axios.get(`/api/chats/${data.id}`); // Replace with your actual chat fetching logic
      setChats(chatsResponse.data); // Set chats for the new user
